...
public interface SysRelateCollectionDefMapper extends BaseMapper<SysRelateCollectionDef> {
    ...
    @Select("SELECT DEV_ID, DATA_TYPE, COLLECT_TYPE_ID,SRC_NAME, I_TYPE_NAME, KEY_ID,\n" +
            ...
            "FROM (SELECT DEV_ID, DATA_TYPE, COLLECT_TYPE_ID, SR.KEY_ID,\n" +
    ...
    @Select("SELECT SEM.`TYPE_NAME`, SEM.`ENUM_MEAN`, SEM.`ENUM_VALUE`, CT.COLLECT_TYPE_ID FROM SYS_ENUM_MEAN_DEF SEM\n" +
            "LEFT JOIN (SELECT * FROM (\n" +
            "SELECT DISTINCT COLLECT_TYPE_ID, TYPE_NAME FROM SYS_RELATE_COLLECTION_DEF  AS SR\n" +
            "LEFT JOIN SYS_ENUM_TYPE_DEF ED ON SR.`COLLECT_TYPE_ID` = ED.`TYPE_ID` WHERE DATA_TYPE = 4) AS T) CT\n" +
            "ON SEM.TYPE_NAME = CT.TYPE_NAME")
